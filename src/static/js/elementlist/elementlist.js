...
(function($){
    let methods = {
        init : function(options){
            return this.each(function(){
                let $this = $(this), data = $this.data("kedu");

                if(!data){
                    data = {
                        select_els: [],
                        kedu_list_multy: false,
                    ...
                    $this.on("mousedown", function(e){
                        ...
                        e = e.originalEvent;
                        let $el = $(e.target);
                        if(data.kedu_list_multy && e.ctrlKey && $el.is("li.selected[data-value]")){
                            data.event_selected = false;
                            $this.KeduList("unSelectItem", $(el));
                        }else if($el.is("li[data-value]")){
                            data.event_selected = true;
                            if(!e.ctrlKey || !data.kedu_list_multy){
                                $this.KeduList("unSelectAllItems");
                            }
                            $this.KeduList("selectItem", $el);
                        }
                    }).on("mouseup", function(){
                        ...
                        if(!data.list_click_hold || !data.kedu_list_multy) return false;
                        e = e.originalEvent;
                        let $el = $(e.target);
        ...
        selectItem: function($el){
            ...
            if($.inArray($el.data("value"), data.select_els) === -1){
                ...
                $this.KeduList("changeSelectList");
            }
            $this.data("kedu", data);
        },
    ...
    $.fn.KeduList = function(method){
        if (methods[method]){
            return methods[ method ].apply( this, Array.prototype.slice.call( arguments, 1 ));
        }else if(typeof method === 'object' || !method){
            return methods.init.apply(this, arguments);
        } else {
            $.error('Метод с именем ' +  method + ' не существует для jQuery.KeduList');
        }
    };
})(jQuery);
